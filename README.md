...
```bash
cd "/Users/madhivananrr/Documents/GUVI/Data Science Final - NEW BATCH MARCH_2026/FINAL PROJECT"
git init
...
Create a `.gitignore` file to prevent uploading large vector store files, local environments, and temporary caches:
...
Export your project dependencies so others can run your code easily:

```bash
cat <<EEOT > requirements.txt
streamlit
pandas
langchain-chroma
...
Create a comprehensive `README.md` file detailing project architecture, installation steps, and usage instructions:

```markdown
# 🛡️ Insurance Policy Assistant (Local RAG)

A free, local, and secure RAG application built using Streamlit, LangChain, Chroma DB, and Meta Llama 3.2 via Ollama.

## 🚀 Features
- **Zero API Costs:** Fully offline architecture using open-source models.
- **Fast Semantic Search:** Powered by Chroma DB and HuggingFace embeddings (`all-MiniLM-L6-v2`).
- **Grounded Responses:** Enforces zero hallucination by relying strictly on retrieved policy context.
- **Streamlit UI:** Interactive dashboard with status badges, one-click FAQ shortcuts, and chat interface.

## 📋 Prerequisites
1. **Ollama**: Install from [ollama.com](https://ollama.com)
2. **Llama 3.2 Model**: Run `ollama pull llama3.2` in terminal.
...
1. Stage and commit your files:
   ```bash
   git add .
   git commit -m "Initial commit: Complete local RAG Insurance Policy Assistant"

```

2. Create a new public repository on [GitHub](https://github.com/new) named `insurance-policy-assistant-rag`.
3. Link your local repository to GitHub and push your code:
```bash
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/insurance-policy-assistant-rag.git
